...
def validate_and_clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        c0 = str(c).strip().lower()
        if c0 in ["interceptions"]:
            rename_map[c] = "interception"
        elif c0 in ["tackles"]:
            rename_map[c] = "tackle"
        elif c0 in ["recoveries"]:
            rename_map[c] = "recovery"
        elif c0 in ["aerial duel", "aerial duels", "aerial_duels"]:
            rename_map[c] = "aerial_duel"
        elif c0 in ["ground duel", "ground duels", "ground_duels"]:
            rename_map[c] = "ground_duel"
        elif c0 in ["clearances"]:
            rename_map[c] = "clearance"
    if rename_map:
...
def _add_pitch_thirds(ax, pitch_mode: str = "rect", pitch_width: float = 64.0,
                      theme: Optional[dict] = None, vertical_pitch: bool = False):
    """Add defensive/middle/attacking thirds to a full-pitch axis."""
    theme = theme or {}
    line_col = theme.get("pitch_lines", theme.get("line", "#E6E6E6"))
    muted = theme.get("muted", theme.get("text", "#A0A7B4"))
    y_max = float(pitch_width if pitch_mode == "rect" else 100.0)

    if vertical_pitch:
        for yv in [100 / 3, 200 / 3]:
            ax.plot([0, y_max], [yv, yv], color=line_col, lw=0.8,
                    ls="--", alpha=0.42, zorder=2)
        for ypos, lbl in [(100 / 6, "Defensive Third"),
                          (50, "Middle Third"),
                          (500 / 6, "Attacking Third")]:
            ax.text(y_max * 0.5, ypos, lbl, color=muted, fontsize=7.5,
                    ha="center", va="center", alpha=0.7, zorder=3)
    else:
        for xv in [100 / 3, 200 / 3]:
            ax.plot([xv, xv], [0, y_max], color=line_col, lw=0.8,
                    ls="--", alpha=0.42, zorder=2)
        for xpos, lbl in [(100 / 6, "Defensive Third"),
                          (50, "Middle Third"),
                          (500 / 6, "Attacking Third")]:
            ax.text(xpos, y_max * 0.975, lbl, color=muted, fontsize=7.5,
                    ha="center", va="top", alpha=0.7, zorder=3)
...
def _filter_legend_handles(handles, legend_cfg: Optional[dict] = None):
    if not legend_cfg:
        return handles
    if not legend_cfg.get("show", legend_cfg.get("show_legend", True)):
        return []
    active = legend_cfg.get("active", None)
    if active is None:
        return handles
    active = set(active)
    active_norm = {str(v).strip().lower().replace(" ", "_") for v in active}
    out = []
    for h in handles:
        label = str(getattr(h, "get_label", lambda: "")()).strip()
        if label in active or label.lower().replace(" ", "_") in active_norm:
            out.append(h)
    return out


def _legend_active_set(legend_cfg: Optional[dict]):
    if not legend_cfg or legend_cfg.get("active", None) is None:
        return None
...
def start_location_heatmap(df: pd.DataFrame, pitch_mode: str = "rect", pitch_width: float = 64.0, theme_name: str = "The Athletic Dark", vertical_pitch: bool = False, show_pitch_thirds: bool = True, legend_cfg: Optional[dict] = None):
    theme = THEMES.get(theme_name, THEMES["The Athletic Dark"])
    ...
    if show_pitch_thirds:
        _add_pitch_thirds(ax, pitch_mode, pitch_width, theme, vertical_pitch)

    handles = [Patch(facecolor=theme.get("muted", "#A0A7B4"), label="Density / Events")]
    _add_legend(ax, handles, theme, loc="upper center", legend_cfg=legend_cfg)
    return fig
...
def defensive_actions_map(
    ...
    if show_pitch_thirds:
        _add_pitch_thirds(ax, pitch_mode, pitch_width, theme, vertical_pitch)
    _add_legend(ax, legend_handles, theme, loc="upper center", legend_cfg=legend_cfg)
    return fig
...
def zone_heatmap(df: pd.DataFrame, pitch_mode: str = "rect", pitch_width: float = 64.0,
                 ...
                 show_pitch_thirds: bool = True, legend_cfg: Optional[dict] = None):
    from matplotlib.patches import Rectangle
    from matplotlib.colors import LinearSegmentedColormap, Normalize
    theme = THEMES.get(theme_name, THEMES["The Athletic Dark"])
...
def carry_map(df: pd.DataFrame, pitch_mode: str = "rect", pitch_width: float = 64.0, theme_name: str = "The Athletic Dark", vertical_pitch: bool = False, show_pitch_thirds: bool = True, legend_cfg: Optional[dict] = None):
    theme = THEMES.get(theme_name, THEMES["The Athletic Dark"])
    ...
    if show_pitch_thirds:
        _add_pitch_thirds(ax, pitch_mode, pitch_width, theme, vertical_pitch)
    _add_legend(ax, [Line2D([0],[0], color=theme.get("text","white"), lw=2, label="Carries")], theme, loc="upper center", legend_cfg=legend_cfg)
    return fig


def receive_map(df: pd.DataFrame, pitch_mode: str = "rect", pitch_width: float = 64.0, theme_name: str = "The Athletic Dark", vertical_pitch: bool = False, show_pitch_thirds: bool = True, legend_cfg: Optional[dict] = None):
    theme = THEMES.get(theme_name, THEMES["The Athletic Dark"])
    ...
    if show_pitch_thirds:
        _add_pitch_thirds(ax, pitch_mode, pitch_width, theme, vertical_pitch)
    _add_legend(ax, [Line2D([0],[0], marker="o", color="none", markerfacecolor=theme.get("text","white"), markersize=8, label="Receives")], theme, loc="upper center", legend_cfg=legend_cfg)
    return fig
...
def numeric_metric_columns(df: pd.DataFrame, exclude_cols: Optional[List[str]] = None, min_valid: int = 1) -> List[str]:
    ...
    for c in df.columns:
        if c in exclude_cols:
            continue
        s = pd.to_numeric(df[c].astype(str).str.replace('%','', regex=False).str.replace(',','', regex=False).str.strip(), errors='coerce')
...
def add_metric_category_colors(metric_categories: List[str], attacking_color: str, possession_color: str, defending_color: str, default_color: str) -> List[str]:
    ...
    for cat in metric_categories:
        ...
        if c.startswith('att') or 'attack' in c:
            colors.append(attacking_color)
        elif c.startswith('pos') or 'possession' in c:
            colors.append(possession_color)
        elif c.startswith('def') or 'defend' in c:
            colors.append(defending_color)
        else:
...
def build_player_metric_table(df: pd.DataFrame, player_col: str, player_name: str, metrics: List[str], value_mode: str = 'Percentile') -> pd.DataFrame:
    d = df.copy()
    for m in metrics:
        d[m] = pd.to_numeric(d[m].astype(str).str.replace('%','', regex=False).str.replace(',','', regex=False).str.strip(), errors='coerce')
...
def generic_scatter_plot(df: pd.DataFrame, x_metric: str, y_metric: str, label_col: Optional[str] = None,
    ...
    d = df.copy()
    d[x_metric] = pd.to_numeric(d[x_metric].astype(str).str.replace('%','', regex=False).str.replace(',','', regex=False).str.strip(), errors='coerce')
    d[y_metric] = pd.to_numeric(d[y_metric].astype(str).str.replace('%','', regex=False).str.replace(',','', regex=False).str.strip(), errors='coerce')
    d = d.dropna(subset=[x_metric, y_metric]).copy()
    fig, ax = plt.subplots(figsize=(9, 7))
    fig.patch.set_facecolor(bg_color); ax.set_facecolor(bg_color)
    ax.scatter(d[x_metric], d[y_metric], s=90, color=x_color, edgecolors='white', linewidth=0.8, alpha=0.82)
    if label_col and label_col in d.columns:
        if highlight_value:
            h = d[d[label_col].astype(str) == str(highlight_value)]
            if not h.empty:
                ax.scatter(h[x_metric], h[y_metric], s=180, color=highlight_color, edgecolors='white', linewidth=1.5, zorder=5)
                for _, r in h.iterrows():
                    ax.text(r[x_metric], r[y_metric], '  ' + str(r[label_col]), color=text_color, fontsize=10, weight='bold', va='center')
        else:
            for _, r in d.head(30).iterrows():
                ax.text(r[x_metric], r[y_metric], '  ' + str(r[label_col]), color=text_color, fontsize=8, alpha=.8)
    ax.axvline(d[x_metric].median(), color=text_color, alpha=.18, ls='--')
    ax.axhline(d[y_metric].median(), color=text_color, alpha=.18, ls='--')
    ax.set_title(title, color=text_color, fontsize=18, weight='bold')
    ax.set_xlabel(x_metric, color=text_color); ax.set_ylabel(y_metric, color=text_color)
...
def generic_bar_chart(metric_table: pd.DataFrame, title: str = 'Bar Chart', value_col: str = 'plot_value',
    ...
    d = metric_table.copy().sort_values(value_col, ascending=True if horizontal else False)
    fig, ax = plt.subplots(figsize=(10, max(5, len(d)*0.45)))
    fig.patch.set_facecolor(bg_color); ax.set_facecolor(bg_color)
    if horizontal:
        ax.barh(d['metric'], d[value_col], color=bar_color)
...
def percentile_bar_chart(metric_table: pd.DataFrame, title: str = 'Percentile Bar',
                         ...
                         bg_color: str = '#0E1117', text_color: str = '#FFFFFF'):
    d = metric_table.copy().sort_values('percentile', ascending=True)
    def c(v):
        return good_color if v >= 75 else mid_color if v >= 50 else low_color
    colors = [c(float(v)) for v in d['percentile']]
    fig, ax = plt.subplots(figsize=(10, max(5, len(d)*0.48)))
    fig.patch.set_facecolor(bg_color); ax.set_facecolor(bg_color)
    ax.barh(d['metric'], d['percentile'], color=colors)
    ax.set_xlim(0, 100)
