...
def ensure_outcome_column(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize column names once at upload stage.
    # This prevents duplicate columns like `Outcome ` + `outcome`,
    # which was the reason for the KeyError in validate_and_clean().
    df.columns = [str(c).strip() for c in df.columns]

    cols_lower = {str(c).strip().lower(): c for c in df.columns}

    # If the file already has Outcome / outcome / Outcome with spaces, rename it safely.
    if "outcome" in cols_lower:
        real_col = cols_lower["outcome"]
        if real_col != "outcome":
            df = df.rename(columns={real_col: "outcome"})
        return df

    # Do NOT use Event as outcome. Event should become event_type, not result/outcome.
    candidates = [
...
with left_col:
    ...
    mode = st.radio(
        "Choose output type",
        ["Match Charts", "Pizza Chart", "Shot Detail Card", "Defensive Actions Map", "Actions Heatmap"],
        horizontal=False,
    )
    ...
    legend_options = ["Outcome", "Heatmaps", "Passes", "Shots", "Touches", "Defensive Actions"]
    ...
    show_column_footer = st.checkbox("Show required columns under each chart", value=True)
    chart_layout_ui = st.selectbox("Charts layout", ["Horizontal", "Vertical"], index=0)
    chart_layout = "vertical" if chart_layout_ui == "Vertical" else "horizontal"
    legend_loc_ui = st.selectbox("Legend position", ["Bottom", "Top", "Right", "None"], index=0)
    legend_loc = legend_loc_ui.lower()
...
with right_col:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # If the uploaded file has an Event column like Shot / Pass / Touch,
        # convert it to event_type so charts classify rows correctly.
        cols_lower_map = {str(c).strip().lower(): c for c in df_raw.columns}
        if "event_type" not in cols_lower_map and "event" in cols_lower_map:
            ev_col = cols_lower_map["event"]
            df_raw["event_type"] = (
                df_raw[ev_col]
                .astype(str)
                .str.strip()
        ...
        if generate_clicked:
            ...
            pdf_path, png_paths = build_report_from_prepared_df(
                ...
                show_column_footer=show_column_footer,
