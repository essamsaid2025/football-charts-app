"""
Football Analysis Suite  v3  — Full modular Streamlit app
Improvements:
- Full legend control (show/hide items, position, styling)
- More themes: Athletic FC, Opta variants, Night Blue, Statsbomb, etc.
- Fixed vertical pitch (full pitch, correct aspect ratio)
- Image overlays on any chart (position, size, circle crop)
- More charts: Goal + Face combo, Convex Hull, Shot Zones, Momentum, Formation map
- Tagging tool fixed (PIL-based, click-to-tag with proper coordinate mapping)
- Opta-style goal location map (Image 1)
- CannonStats-style goal mouth map (Image 2)
"""
...
def _load_local_module(alias, filename):
    ...
    for search_path in [os.path.dirname(__file__), "/home/claude", "/mnt/project"]:
...
from charts_extra import (
    ...
    draw_tagging_pitch, EXTRA_THEMES,
)
...
LEGEND_POSITIONS = ["lower center","upper center","lower left","upper left",
...
def req_badge(cols):
    ...
    st.markdown(f"<div style='margin-bottom:8px'><b style='color:#6b8cae;font-size:.78rem'>Requires:</b> {badges}</div>",
                unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
# FULL LEGEND CONTROLS  – returns dict with active items + styling
# ─────────────────────────────────────────────────────────────────────────────
def legend_controls_full(prefix, all_items: list, default_active: list = None) -> dict:
    """Full legend customization: show/hide items, position, font size, title."""
    ...
    return dict(active=active, pos=pos, fontsize=fs, title=title_txt or None, frame=frame)


def apply_legend_style(ax, legend_cfg: dict, theme: dict):
    """Apply custom legend to a matplotlib axis."""
    ...
    leg = ax.legend(
        ...
        bbox_to_anchor=(0.5, -0.02) if "center" in legend_cfg.get("pos","lower center") else None,
    )
...
def img_overlay_controls(prefix, label="Overlay Image",
    ...
    with st.expander(f"🖼️ {label}", expanded=False):
        ...
        if img:
            ...
            with col_x: x = st.slider("X position (fig %)", 0, 95, int(default_x*100), key=f"{prefix}_ix") / 100
            with col_y: y = st.slider("Y position (fig %)", 0, 95, int(default_y*100), key=f"{prefix}_iy") / 100
            col_w, col_h = st.columns(2)
            with col_w: w = st.slider("Width (fig %)", 4, 30, int(default_w*100), key=f"{prefix}_iw") / 100
            with col_h: h = st.slider("Height (fig %)", 4, 30, int(default_h*100), key=f"{prefix}_ih") / 100
            col_o, col_c = st.columns(2)
            with col_o: alpha_v = st.slider("Opacity", 10, 100, 100, key=f"{prefix}_ialpha") / 100
            with col_c: circle = st.checkbox("Circle crop", False, key=f"{prefix}_ic")
            bdr_lw = st.slider("Border width", 0.0, 5.0, 0.0, 0.5, key=f"{prefix}_ib")
        else:
            x, y, w, h, circle, bdr_lw, alpha_v = default_x, default_y, default_w, default_h, False, 0.0, 1.0
    return dict(img=img, x=x, y=y, w=w, h=h, circle=circle, bdr_lw=bdr_lw, alpha=alpha_v)
...
def apply_overlay(fig, ov):
    if ov.get("img"):
        try:
            img = ov["img"].copy()
            if ov.get("alpha", 1.0) < 1.0:
                r, g, b, a = img.split()
                a = a.point(lambda p: int(p * ov["alpha"]))
                img = Image.merge("RGBA", (r, g, b, a))
            overlay_image_on_fig(
                fig, img, x=ov["x"], y=ov["y"], w=ov["w"], h=ov["h"],
                circle_crop=ov["circle"], border_lw=ov.get("bdr_lw", 0.0))
        except Exception:
            pass


# ─────────────────────────────────────────────────────────────────────────────
# EVENT DATA SETUP  (used by all pitch-map sections)
# ─────────────────────────────────────────────────────────────────────────────
...
def nofile():
    empty("📂", "No file uploaded", "Upload an event-data file from the sidebar.")
...
# ─────────────────────────────────────────────────────────────────────────────
# NEW: OPTA-STYLE GOAL LOCATION + FACE COMBO MAP  (Image 1 reference)
# ─────────────────────────────────────────────────────────────────────────────
def opta_goal_location_combo(
    df, title="Player Name", subtitle="Club | League Season",
    player_img=None, logo_img=None,
    goal_color="#C8102E", goal_edge="#8B0000",
    dot_size=160, penalty_label_col=None,
    theme_name="Opta Analyst Light",
    pitch_mode="rect", pitch_width=68.0,
    stat_labels=None,
    show_type_breakdown=True,
):
    """
    Opta Analyst-style: half-pitch goal location map + right-side stats panel.
    Mirrors Image 1 (Mohamed Salah style).
    Requires: x, y, outcome
    Optional: body_part / type_col for header/foot breakdown
    """
    ...
    # Divider
    ax_div = fig.add_axes([0.0, 0.865, 1.0, 0.006])
    ax_div.set_facecolor(muted_col); ax_div.axis("off")

    # Pitch panel (left ~60%)
    ax_pitch = fig.add_axes([0.01, 0.04, 0.60, 0.81])
    ...
    # Arc
    arc_cx = 100 - pa_l
    arc_rx = 9.15/105.0*100.0; arc_ry = 9.15/68.0*y_max
    theta = np.linspace(np.pi*0.62, np.pi*1.38, 80)
    axs = arc_cx + arc_rx*np.cos(theta); ays = mid + arc_ry*np.sin(theta)
    out_m = axs <= (100-pa_l+0.5)
    ax_pitch.plot(axs[out_m], ays[out_m], color=lc, lw=lw)
    # Halfway arc
    arc2 = np.linspace(-np.pi/2, np.pi/2, 60)
    ax_pitch.plot(50 + 9.15*np.cos(arc2), mid + (9.15/68.0*y_max)*np.sin(arc2), color=lc, lw=lw)
    ...
    if player_img:
        overlay_image_on_fig(fig, player_img, x=0.01, y=0.89, w=0.07, h=0.09,
                             circle_crop=True, border_lw=0.0)
    if logo_img:
        overlay_image_on_fig(fig, logo_img, x=0.87, y=0.89, w=0.10, h=0.09,
                             circle_crop=False, border_lw=0.0)
...
def shot_zone_map(df, theme_name="The Athletic Dark", pitch_mode="rect",
    ...
    """Divide pitch into zones, count shots & xG per zone. Requires: x,y,outcome,xg"""
    ...
    zones = {
        "6-yard box":    (94, y_max*0.5-y_max*0.054, 100, y_max*0.5+y_max*0.054),
        "Penalty spot":  (88, y_max*0.5-y_max*0.135, 94, y_max*0.5+y_max*0.135),
        "Central zone":  (78, y_max*0.5-y_max*0.20,  88, y_max*0.5+y_max*0.20),
        "Left channel":  (78, y_max*0.5-y_max*0.40,  100,y_max*0.5-y_max*0.20),
        "Right channel": (78, y_max*0.5+y_max*0.20,  100,y_max*0.5+y_max*0.40),
        "Long range L":  (55, y_max*0.5-y_max*0.50,  78, y_max*0.5-y_max*0.15),
        "Long range C":  (55, y_max*0.5-y_max*0.15,  78, y_max*0.5+y_max*0.15),
        "Long range R":  (55, y_max*0.5+y_max*0.15,  78, y_max*0.5+y_max*0.50),
    }
    ...
    cmap = LinearSegmentedColormap.from_list("shots", ["#0B2A4A","#1A78CF","#FF9300","#C8102E"])
...
def momentum_chart(df, team_a="Home", team_b="Away", team_col=None,
    ...
    """Rolling event count momentum chart. Requires: minute column + (optional) team column."""
    theme = THEMES.get(theme_name, THEMES["The Athletic Dark"])
    ...
    if team_col and team_col in d.columns:
        for team_name, color, sign in [(team_a, color_a, 1), (team_b, color_b, -1)]:
            td = d[d[team_col].astype(str) == str(team_name)]
            counts, _ = np.histogram(td[min_c], bins=bins)
            xs = (bins[:-1] + bins[1:]) / 2
            ys = counts * sign
            ax.bar(xs, ys, width=4.5, color=color, alpha=0.75, label=team_name)
    else:
...
# ─────────────────────────────────────────────────────────────────────────────
# FIXED VERTICAL PITCH helper  (correct full-pitch sizing)
# ─────────────────────────────────────────────────────────────────────────────
def make_full_vertical_pitch(pitch_mode, pitch_width, theme):
    """Returns (fig, ax) for a full vertical pitch with correct aspect ratio."""
    ...
    y_max = float(pitch_width if pitch_mode == "rect" else 100.0)
    fig_w = 8.0
    # Correct aspect: pitch is 100 long × y_max wide → height = fig_w × (100/y_max)
...
# ─────────────────────────────────────────────────────────────────────────────
if section == "🏠 Home":
    hdr("⚽","Football Analysis Suite v3","Professional charts · scouting · tagging — fully modular")
    tiles = [
        ("⚔️","Attacking Charts","Shot maps, Opta goal location, xG timeline, shot zones","x, y, outcome"),
        ("🛡️","Defensive Charts","Defensive action maps, regains heatmap, pressure map, momentum","x, y + defensive cols"),
        ("🔄","Distribution Charts","Pass maps, progressive carries, passing network","x, y, x2, y2, outcome"),
        ("🎯","Specialist Charts","Goal mouth (CannonStats), vertical maps, shot detail card","x, y, outcome, z/height"),
        ("🍕","Radars & Pizza","MPL pizza, Athletic style, percentile bars, scatter plots","Player col + metrics"),
        ("🧠","Player Scouting","Role templates, scoring, shortlists, scout reports","Player col + metrics"),
        ("🖱️","Tagging Tool","Click-to-tag events on an interactive pitch","None — draw on pitch"),
    ]
    cols = st.columns(3)
    ...
    st.markdown("""
**What's new in v3**
- **Opta-style goal location map** (Image 1 — player name, stats panel, dot-per-goal on half pitch)
- **CannonStats goal mouth map** (Image 2 — shots plotted on goal face, sized by xG)
- **Shot Zones map** — pitch divided into zones with shot count + xG per zone
- **Momentum chart** — rolling event/action count across match minutes
- **Full legend control** — show/hide individual items, set position, font size, title
- **Image overlays on every chart** — position, size, opacity, circle crop
- **More themes** — Opta Analyst Light/Pink, The Athletic FC Light/Dark, Night Blue, Broadcast Green, Statsbomb Light, Whoscored Dark
- **Vertical pitch fixed** — full pitch with correct aspect ratio
- **Tagging tool fixed** — PIL-based click-to-tag with proper coordinate mapping
""")
    st.stop()
...
if section == "⚔️ Attacking Charts":
    df, S = ev_sidebar("atk")
    hdr("⚔️","Attacking Charts","Shot map · Opta goal location · Shot zones · Touch map · xG Timeline")

    tabs = st.tabs(["Shot Map","Opta Goal Location","Shot Zones","Touch Map","Start Heatmap","xG Timeline"])
        ...
        if df is None: nofile()
            ...
            with R:
                if gen:
                    ...
                    sm_f = {k:v for k,v in sm.items() if k in leg_cfg["active"]}
                    ...
                    apply_legend_style(fig.axes[0], leg_cfg, THEMES[S["tn"]])
    ...
    with tabs[1]:
        ...
        if df is None: nofile()
            ...
            with L:
                ...
                st.caption("Plots each goal as a dot on a half-pitch with a stats panel on the right (Opta Analyst style).")
                ...
                st.markdown("**Stats panel** (up to 6 rows — value then label)")
                ...
                player_ov = img_overlay_controls("gl_plyr","Player image (top-left)",default_x=0.01,default_y=0.89,default_w=0.07,default_h=0.09)
                logo_ov   = img_overlay_controls("gl_logo","Logo image (top-right)",default_x=0.87,default_y=0.89,default_w=0.10,default_h=0.09)
                gen = st.button("Generate",key="gl_gen")
                st.markdown('</div>',unsafe_allow_html=True)
            with R:
                if gen:
                    fig = opta_goal_location_combo(
                        df, title=gl_title, subtitle=gl_sub,
                        player_img=player_ov.get("img"), logo_img=logo_ov.get("img"),
                        goal_color=gl_color, goal_edge=gl_edge, dot_size=gl_ds,
                        theme_name=gl_tn, pitch_mode=S["pm"], pitch_width=S["pw"],
                        stat_labels=stats or None,
                    )
                    st.session_state["atk_gl"] = fig
                if "atk_gl" in st.session_state: preview(st.session_state["atk_gl"],"opta_goal_location")
                else: empty("🥅","Configure and generate")
    ...
    with tabs[2]:
        ...
        if df is None: nofile()
            ...
            with R:
                ...
                if "atk_sz" in st.session_state: preview(st.session_state["atk_sz"],"shot_zones")
            ...
            with R:
                if gen:
                    if tm_v:
                        ...
                        vp.scatter(d2["x"], d2["y"], ax=ax, s=tm_s, marker=MARKER_OPTS[tm_ml],
                                   color=tm_c, edgecolors=tm_e, linewidth=2, alpha=tm_a, zorder=5)
                        ax.set_title(tm_t, color=THEMES[S["tn"]]["text"], fontsize=16, weight="bold")
                    else:
            ...
            with R:
                if gen:
                    if ht_v:
                        ...
                        ax.set_title(ht_t, color=THEMES[S["tn"]]["text"], fontsize=16, weight="bold")
                    else:
                        fig = start_location_heatmap(df, pitch_mode=S["pm"], pitch_width=S["pw"],
                                                     theme_name=S["tn"], vertical_pitch=False)
...
if section == "🛡️ Defensive Charts":
    df, S = ev_sidebar("def")
    hdr("🛡️","Defensive Charts","Actions map · Regains heatmap · Pressure map · Momentum · Outcome bar")

    tabs = st.tabs(["Defensive Actions","Ball Regains Map","Pressure Map","Momentum Chart","Outcome Distribution"])

    with tabs[0]:
        req_badge(["x","y","outcome","interception/tackle/recovery/..."])
        if df is None: nofile()
        else:
            L, R = st.columns([1, 2.2])
            with L:
    ...
    with tabs[1]:
        req_badge(["x","y","interception/tackle/recovery/..."])
        if df is None: nofile()
        else:
            L, R = st.columns([1, 2.2])
            with L:
            ...
            with R:
                if gen:
                    ...
                    fig = defensive_regains_map(df, title=t, def_colors=dc_f, def_markers=dm_f,
                                                pitch_mode=S["pm"], pitch_width=S["pw"],
                                                theme_name=S["tn"], vertical_pitch=False,
                                                marker_size=bms, zone_alpha=bal, show_zone_values=bz)
                    apply_overlay(fig, ov)
    ...
    with tabs[2]:
        req_badge(["x","y","pressure (bool col, optional)"])
        if df is None: nofile()
        else:
            L, R = st.columns([1, 2.2])
            with L:
                ...
                if gen:
                    if pv:
                        ...
                        try:
                            ...
                            vp.scatter(d2["x"], d2["y"], ax=ax, s=30, alpha=0.4, color="#FF4060")
                        ax.set_title(t, color=THEMES[S["tn"]]["text"], fontsize=16, weight="bold")
                    else:
                        fig = pressure_map(df, title=t, theme_name=S["tn"],
                                           pitch_mode=S["pm"], pitch_width=S["pw"],
                                           vertical_pitch=False, pressure_col=pc or "pressure")
                    apply_overlay(fig, ov)
    ...
    with tabs[3]:
        ...
        if df is None: nofile()
            ...
            with L:
                ...
                mm_ca = st.color_picker("Team A color","#00C2FF",key="mm_ca")
                mm_cb = st.color_picker("Team B color","#FF4060",key="mm_cb")
                        ...
                        for outc in PASS_ORDER:
                            ...
                            if has_end.any():
                                vp_p.arrows(sub.loc[has_end,"x"],sub.loc[has_end,"y"],
                                            ...
                                            ax=ax, color=pc_f.get(outc,"#AAAAAA"), width=1.8, alpha=0.8)
                            vp_p.scatter(sub["x"],sub["y"],ax=ax,s=70,
                                         color=pc_f.get(outc,"#AAAAAA"),edgecolors="white",linewidth=0.8,zorder=5)
                        ax.set_title(t, color=THEMES[S["tn"]]["text"], fontsize=16, weight="bold")
                    else:
                        fig = pass_map(df, pass_colors=pc_f, pass_markers=pm_f,
                                       pitch_mode=S["pm"], pitch_width=S["pw"],
                                       theme_name=S["tn"], vertical_pitch=False,
                                       pass_view=pv, result_scope=ps, min_packing=ppk)
                        fig.axes[0].set_title(t, color=THEMES[S["tn"]]["text"], fontsize=16, weight="bold")
                        apply_legend_style(fig.axes[0], leg_cfg, THEMES[S["tn"]])
                    apply_overlay(fig, ov)
    ...
    with tabs[1]:
        ...
        if df is None: nofile()
            ...
            with L:
                ...
                st.caption("Requires x,y,x2,y2. Set event_type='carry' or all rows are used.")
            ...
            with R:
                if gen:
                    if dv:
                        fig, ax, vp_c = make_full_vertical_pitch(S["pm"], S["pw"], THEMES[S["tn"]])
                        d2 = df.copy()
                        if "event_type" in d2.columns:
                            d2 = d2[d2["event_type"].astype(str).str.lower()=="carry"]
                        for c_ in ["x","y","x2","y2"]:
                            if c_ in d2.columns: d2[c_] = pd.to_numeric(d2[c_], errors="coerce")
                        d2 = d2.dropna(subset=["x","y","x2","y2"])
                        prog = d2[d2["x2"]-d2["x"] >= md]
                        if not prog.empty:
                            vp_c.arrows(prog["x"],prog["y"],prog["x2"],prog["y2"],
                                        ax=ax, color=cc, width=2.0, headwidth=6, alpha=0.88, zorder=4)
                            vp_c.scatter(prog["x"],prog["y"],ax=ax,s=55,color=cc,
                                         edgecolors="white",linewidth=0.8,zorder=5)
                        ax.set_title(t, color=THEMES[S["tn"]]["text"], fontsize=16, weight="bold")
                    else:
                        fig = progressive_carries_map(df, title=t, theme_name=S["tn"],
                                                      pitch_mode=S["pm"], pitch_width=S["pw"],
                                                      carry_color=cc, min_distance=md, vertical_pitch=False)
                    apply_overlay(fig, ov)
...
if section == "🎯 Specialist Charts":
    ...
    hdr("🎯","Specialist Charts","CannonStats goal mouth · Combined shot report · Vertical maps · Shot card")

    tabs = st.tabs(["Goal Mouth Map","Shot Report (Combined)","Vertical Pitch Map","Shot Detail Card"])
    ...
    with tabs[0]:
        ...
        if df is None: nofile()
            ...
            with L:
                ...
                st.caption("Plots shots on target on the goal face. Sized by xG. Mirrors Image 2.")
                ...
                all_leg_items = ["Save","Goal","Post-Shot xG Value"]
                leg_cfg_gm = legend_controls_full("gm", all_leg_items)
    ...
    with tabs[1]:
        ...
        if df is None: nofile()
            ...
            with L:
                ...
                st.caption("Half-pitch goal location + goal face inset panel.")
                ...
                all_gsr_items = ["Pitch goals","Goal","Save","xG size"]
                leg_cfg_gsr = legend_controls_full("gsr", all_gsr_items)
    ...
    with tabs[2]:
        ...
        if df is None: nofile()
            ...
            L, R = st.columns([1, 2.2])
            with L:
                st.markdown('<div class="card"><div class="ctitle">Vertical Pitch Map (Fixed Full Pitch)</div>', unsafe_allow_html=True)
                ...
                leg_cfg = legend_controls_full("vp", all_et_items)
                ov  = img_overlay_controls("vp_ov")
                gen = st.button("Generate",key="vp_gen")
                st.markdown('</div>',unsafe_allow_html=True)
            with R:
                if gen:
                    # Use the FIXED full vertical pitch
                    fig, ax, vp2 = make_full_vertical_pitch(S["pm"], S["pw"], THEMES[S["tn"]])
                    et_lower = et.lower()
    ...
    with tabs[3]:
        ...
        if df is None: nofile()
            ...
            if shots.empty: st.warning("No shots found in data.")
            else:
                def _sf(v):
                    try: return float(v)
                    except: return float("nan")
                shots["label"] = shots.apply(
                    lambda r: f'{r.name+1} | {str(r["outcome"]).upper()} | xG {_sf(r.get("xg","0")):.2f}',
                    axis=1)
                L, R = st.columns([1, 2.2])
                with L:
        ...
        with tab:
            ...
            if ck in ["mpl","ath","pb"]:
                ...
                with L:
                    ...
                    sx   = st.selectbox("X-axis metric",mc_,index=0,key="sc_x")
                    sy   = st.selectbox("Y-axis metric",mc_,index=min(1,len(mc_)-1),key="sc_y")
...
if section == "🧠 Player Scouting":
    ...
    def _ir(pos):
        ...
        mapping = {"gk":"Goalkeeper","goalkeeper":"Goalkeeper","cb":"Centre Back",
                   "rcb":"Centre Back","lcb":"Centre Back","rb":"Full Back / Wing Back",
                   "lb":"Full Back / Wing Back","rwb":"Full Back / Wing Back",
                   "lwb":"Full Back / Wing Back","dm":"Defensive Midfielder",
                   "cdm":"Defensive Midfielder","cm":"Central Midfielder","mc":"Central Midfielder",
                   "am":"Attacking Midfielder","cam":"Attacking Midfielder",
                   "rw":"Winger","lw":"Winger","st":"Striker","cf":"Striker","striker":"Striker"}
    ...
    def match_template_metrics(metrics_list, template_metrics):
    ...
    metrics_ = numeric_metrics(dfr, [pcol,tcol,poscol,agcol,mincol,valcol])
    ...
    matched_ = match_template_metrics(metrics_, template_metrics_)
    ...
    c2.metric("Metrics", len(selected_))
    c3.metric("Top score", "-" if scored.empty else scored["Adjusted Score"].iloc[0])

    show_cols = [c for c in [pcol,tcol,poscol,agcol,mincol,valcol,"Scouting Score","Reliability","Adjusted Score"]
...
# ─────────────────────────────────────────────────────────────────────────────
# 🖱️  TAGGING TOOL  — FIXED
# ─────────────────────────────────────────────────────────────────────────────
if section == "🖱️ Tagging Tool":
    hdr("🖱️","Tagging Tool","Click the pitch to tag events, passes, carries, shots and more")
...
To enable click-to-tag, add `streamlit-image-coordinates` to your `requirements.txt`
and restart the app.

In the meantime you can still use the **Manual Entry** tab below to log events by typing coordinates.
""")
    ...
    with L:
        ...
        if two_click:
            st.info(f"**{event_type.title()}**: click START point, then END point.")
        ...
        with c3:
            if two_click and st.session_state["tag_start"]:
                if st.button("✖ Cancel start",key="tag_cancel"):
                    st.session_state["tag_start"] = None
                    st.rerun()
        ...
        if two_click:
            if st.session_state["tag_start"]:
                ...
                st.success(f"Start: ({sp[0]:.1f}, {sp[1]:.1f}) — now click END point")
            else:
                st.info("Click pitch to set START point")
        st.markdown('</div>', unsafe_allow_html=True)

        # Manual entry fallback
        with st.expander("✏️ Manual Event Entry (no-click fallback)"):
            me_x  = st.number_input("X",0.0,100.0,50.0,key="me_x")
            me_y  = st.number_input("Y",0.0,float(pw_tag),float(pw_tag)/2,key="me_y")
            me_x2 = st.number_input("X2 (end)",0.0,100.0,60.0,key="me_x2") if two_click else None
            me_y2 = st.number_input("Y2 (end)",0.0,float(pw_tag),float(pw_tag)/2,key="me_y2") if two_click else None
            if st.button("Add Event",key="me_add"):
                ev = {"event_type":event_type,"outcome":outcome,
                      ...
                      "start_marker":marker_ev,"start_size":size_ev,"arrow_color":col_ev}
    ...
    with R:
        ...
        try:
            ...
            st.error(f"Pitch render error: {e}")
            st.stop()
        ...
        if HAS_SIC:
            ...
            if click and "x" in click and "y" in click:
                ...
                if key_c != st.session_state["tag_last_click"]:
                    ...
                    inner_w = img_w - 2 * pad_tag
                    inner_h = img_h - 2 * pad_tag
                    # Convert pixel → pitch coordinates
                    ...
                    if two_click and st.session_state["tag_start"] is None:
                        ...
                        # Second click (or single-click event): commit
                        ev = {
                            "event_type": event_type, "outcome": outcome,
                            "x": x_pitch, "y": y_pitch,
                            "player": player, "team": team, "minute": minute,
                            "xg": xg_val if event_type=="shot" else 0,
                            "start_color": col_ev, "start_edge": edge_ev,
                        ...
                        if two_click and st.session_state["tag_start"] is not None:
                            ...
                            ev["x"] = sx; ev["y"] = sy
                            ev["x2"] = x_pitch; ev["y2"] = y_pitch
                            st.session_state["tag_start"] = None
                        st.session_state["tag_events"].append(ev)
                    st.rerun()
        else:
            # Show static pitch when no SIC
            buf = io.BytesIO()
        ...
        if not events_df.empty:
            ...
            # Quick pitch preview from tagged data
